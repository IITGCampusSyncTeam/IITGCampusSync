...
import 'package:shared_preferences/shared_preferences.dart';
  ...
  class _CalendarScreenState extends State<CalendarScreen> {
    ...
    Widget _buildCurrentView(EventProvider provider) {
      if (_userId == null) {
        return const Center(child: CircularProgressIndicator()); // Wait for userID
      }

      List<Event> filteredEvents = _showMyEventsOnly
          ? provider.allEvents.where((event) =>
          event.participants.contains(_userId)).toList()
          : provider.allEvents;
      ...
      switch (_currentView) {
        ...
        case ViewType.calendar:
          return buildCalendarView(
            provider.getEventsForMonth(provider.selectedDate.year, provider.selectedDate.month),
